...
function prices(options, optionalHttpRequestOptions, cb) {
  if (_.isUndefined(options)) {
    options = {};
  }
  _sanitizeHistorical(options);
  ...
  return Promise.resolve(symbols)
    .map(function(symbol) {
      return Promise.resolve()
        .then(function() {
          var url = 'https://www.google.com/finance/getprices';
          var [x, q] = symbol.split(':');
          var qs = {
            q: q,
            x: x,
            i: options.interval || 300, //86399, //
            p: options.period || '1d' //'1M' //
          };

          return _utils.download(url, qs, optionalHttpRequestOptions);
        }).then(
          function(data) {
            return convertToTickData(data, symbol);
          })
        .catch(function(err) {
          if (options.error) {
            throw err;
...
function convertToTickData(text, symbol) {
  ...
  for (var i = 0; i < tickArray.length; i++) {
    ...
    var date = utc.format('YYYY-MM-DD[T]HH:mm:ss.SSS[Z]');
